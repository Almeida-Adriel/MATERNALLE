...
const Acompanhamento = ({ dados }) => {
  ...
  const calculaMesSemanaBebe = (dataNascimento) => {
    ...
    const MAX_WEEKS = 96; // 24 meses * 4 semanas/mês
    if (totalWeeks >= MAX_WEEKS) {
      return '4-24'; // Última chave válida
    }
  ...
  const next = () => {
    setCurrentIndex(
      (prevIndex) => (prevIndex + 1) % (isGestante ? 1 : filhos.length)
    );
  };
  ...
  const prev = () => {
    setCurrentIndex(
      (prevIndex) =>
        (prevIndex - 1 + (isGestante ? 1 : filhos.length)) %
        (isGestante ? 1 : filhos.length)
    );
  };
  const textLink = () => {
            ...
            <>
              <div className="space-y-4">
                ...
                <p className="text-lg text-slate-700">
                  <strong>Status da Maternidade:</strong>{' '}
                  {userData.status_maternidade}
                </p>
                <p className="text-lg text-slate-700">
                  <strong>Data de Previsão do Parto:</strong>{' '}
                  {new Date(userData.dpp).toLocaleDateString('pt-BR')}
                </p>
                <p className="text-lg text-slate-700">
                  <strong>Contagem Regressiva para o parto:</strong>{' '}
                  {contagemRegressivaDPP(userData.dpp)}
                </p>
                <p className="text-lg text-slate-700">
              ...
              <div className="mt-6">
                <h4 className="text-xl font-semibold text-brand-700 mb-2">
                  Dica da Semana
                </h4>
                {dica ? (
                  <div className="p-4 bg-brand-50 rounded border border-brand-100">
                    ...
                    {dica.link && (
                      <a
                        ...
                        className="text-brand-600 underline mt-2 block"
                      >
                        Ver estudo
                      </a>
                    )}
                  ...
                  <p className="text-slate-500 italic">
                    Sem dica disponível para esta semana.
                  </p>
                )}
              </div>
            </>
            ...
            <>
              <div className="space-y-4">
                ...
                <p className="text-lg text-slate-700">
                  ...
                  {new Date(
                    filhos[currentIndex]?.data_nascimento
                  ).toLocaleDateString('pt-BR')}
                </p>
                <p className="text-lg text-slate-700">
                  <strong>Idade:</strong>{' '}
                  {idadeFormatada(filhos[currentIndex]?.data_nascimento)}
                </p>
                <p className="text-lg text-slate-700">
                  <strong>Gênero:</strong>{' '}
                  {filhos[currentIndex]?.genero || 'Não informado'}
                </p>
                <p className="text-lg text-slate-700">
                  <strong>Peso ao Nascer:</strong>{' '}
                  {filhos[currentIndex]?.peso_nascimento || 'Não informado'}
                </p>
                <p className="text-lg text-slate-700">
                  <strong>Tipo de Parto:</strong>{' '}
                  {filhos[currentIndex]?.tipo_parto || 'Não informado'}
                </p>
              </div>
              <div className="mt-6">
                <h4 className="text-xl font-semibold text-brand-700 mb-2">
